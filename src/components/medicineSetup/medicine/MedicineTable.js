...
function MedicineTable({ setData, reload }) {
	...
	const COLUMNS = [
		...
		{
			Header: "Generic Name",
			accessor: "medicineGeneric.genericName",
			Filter: ColumnFilter,
			Cell: (row) => <> {row.row.original.medicineGeneric?.genericName} </>,
		},
		{
			Header: "Strength",
			accessor: "strength",
		...
		},
		{
			...
			Filter: ColumnFilter,
			Cell: (row) => <> {row.row.original.category?.categoryName} </>,
		},
		{
			Header: "Manufacturer",
			accessor: "medicineGeneric.manufacturer",
			Filter: ColumnFilter,
			Cell: (row) => (
				<> {row.row.original.medicineManufacturer?.manufacturerName} </>
			),
		},
		// {
		// 	Header: "Description",
	...
	const [medicine, isLoading, , refetch] = useFetch(
	...
	const data = useMemo(() => medicine, [medicine]);
