...
function DiagnosisEditForm({ data, setData, reload, setReload }) {
	const {
		register,
		handleSubmit,
		formState: { errors },
	...
	const onSubmit = (diagnosisData) => {
		setLoading(true);
		fetch(`${process.env.REACT_APP_API_URL}/api/diagnosis/${data.id}`, {
			method: "PATCH",
			headers: {
		...
		<form onSubmit={handleSubmit(onSubmit)}>
			<Box padding="10px">
				...
				<Box padding="20px 0">
					...
					<Box marginBottom="10px">
						<Typography variant="p">Complaint Name</Typography>
						<TextField
