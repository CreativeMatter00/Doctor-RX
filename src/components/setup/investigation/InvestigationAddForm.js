...
function InvestigationAddForm({ reload, setReload }) {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const onSubmit = (data) => {
    setLoading(true);
    fetch(`${process.env.REACT_APP_API_URL}/api/investigation`, {
      method: "POST",
      headers: {
    ...
    <form onSubmit={handleSubmit(onSubmit)}>
      <Box padding="10px">
        ...
        <Box padding="20px 0">
          ...
          <Box marginBottom="10px">
            <Typography variant="p">Investigation Name</Typography>
            <TextField
...
export default InvestigationAddForm;
