...
function InvestigationEditForm({ data, setData, reload, setReload }) {
	const {
		register,
		handleSubmit,
		formState: { errors },
	...
	const onSubmit = (investigationData) => {
		setLoading(true);
		fetch(`${process.env.REACT_APP_API_URL}/api/investigation/${data.id}`, {
			method: "PATCH",
			headers: {
